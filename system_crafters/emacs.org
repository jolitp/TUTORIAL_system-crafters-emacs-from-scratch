...
* DONE Emacs From Scratch #4 - Projectile and Magit
** projectile
...
important functions in projectile:
I reach that with =SPC p=
...
the above 2 command don't do anything by default

*** directory local variables

helpful to configure emacs or projectile for a specific project

create a file inside project folder that configures emacs variables
so when you load a buffer inside that folder it automatically
apply those variables to your buffer

you can create that file from projectile:
=E projectile-edit-dir-locas=

that opens a new buffer with a file called =.dir-locals.el=

it shows a list of possible variables

then it asks for the variable's value

then it adds to the file

NOTE: when you edit the =.dir-locals.el= file it does not
reload the variables in the oppened emacs 

*** counsel projectile ripgrep

counsel already has the ability to run searching tools like ripgrep
giving a list of results for running grep over a particular folder

with projectile it gives the ability to grep over current project

use it by running:

=counsel-projectile-rg=

by using ivy you can press =C-c C-o= when on the list
that will open a separate buffer where you can select text
and use swiper

pressing =ENTER= on any item will open the loction of the result
on the buffer you were when you called =counsel-projectile-rg=

the list that you get in ivy by pressing =C-o=
in that new buffer you just need to press =o=

to close that buffer press =q=
** magit
...
*** evil-magit
is unnecessary, it is part of evil-collection now

*** deleting files from magit

put the cursor over the file and press =x=
that will discard changes to the file

*** a better alternative to rebase in emacs and magit

when commiting, instead of pressing =c c=

use =instant fixup= by pressin =F=

that gives a list of recent commits

by pressin =C-c C-c= on a line (a recent commit)

that will add the recent commit to the current commit(is that it?)

*** actual rebase

you can do that too

he explains that in detail, but I don't know enough about rebase to understand...

** forge

forge lets you pull down all the information from a github repository,
like issues and pull requests

you can interact with those from emacs
...
for each project on github you need to run =forge-pull=

after that you will see on magit screen a =issues= section
...
* TODO Emacs From Scratch #5 - Org Mode Basics
** org mode manual
...
** DONE have more than one projectile search path
how to have a list of directories in:

=(setq projectile-project-search-path '("~/Projects"))=

is it just putting another item? like
...
** load .zshrc
** use zsh in emacs
** when switching projectile projects to directly to the readme
show the readme if thats available
** setup emacs to ask for passphrases
like when you have a ssh key with a passphrase
...
evil-collection.
* todos
** TODO go back on the videos and take notes of the tips and tricks he says
** TODO get link for documentation of each package
